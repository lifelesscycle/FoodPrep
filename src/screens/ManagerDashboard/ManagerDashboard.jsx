...
const ManagerDashboard = () => {
  ...
  const API_BASE = 'http://localhost:8000'; // Adjust according to your FastAPI server
  ...
  // Fetch all orders for each status
  const fetchAllOrders = async () => {
    ...
    try {
      ...
      for (const status of orderStatuses) {
        const response = await fetch(`${API_BASE}/orders_by_status?status=${status.value}`);
        if (response.ok) {
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await fetch(`${API_BASE}/order_analytics`);
      if (response.ok) {
